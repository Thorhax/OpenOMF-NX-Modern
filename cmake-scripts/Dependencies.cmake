...
if(CMAKE_SYSTEM_NAME STREQUAL "NintendoSwitch")
    ...
    if(NOT TARGET SDL2::Core)
        ...
        target_link_libraries(SDL2::Core INTERFACE SDL2::SDL2)
    endif()
    if(NOT TARGET SDL2::Main)
        add_library(SDL2::Main INTERFACE IMPORTED)
        target_link_libraries(SDL2::Main INTERFACE SDL2::SDL2main)
    endif()
    target_link_libraries(openomf::SDL2 INTERFACE SDL2::SDL2)
    target_link_libraries(openomf::SDL2main INTERFACE SDL2::SDL2main openomf::SDL2)

    find_package(SDL2_mixer REQUIRED)
    target_link_libraries(openomf::SDL2_mixer INTERFACE SDL2::Mixer mpg123 vorbisidec modplug stdc++ opusfile opus ogg)
...
if(TARGET xmp_static)
    target_link_libraries(openomf::xmp INTERFACE xmp_static)
    target_include_directories(openomf::xmp INTERFACE "${CMAKE_SOURCE_DIR}/3rdparty/libxmp/include")
elseif(EXISTS "${CMAKE_SOURCE_DIR}/3rdparty/libxmp/CMakeLists.txt")
    set(BUILD_STATIC ON CACHE BOOL "" FORCE)
    set(BUILD_SHARED OFF CACHE BOOL "" FORCE)
    set(BUILD_LITE OFF CACHE BOOL "" FORCE)
    set(LIBXMP_PIC ON CACHE BOOL "" FORCE)
    add_subdirectory("${CMAKE_SOURCE_DIR}/3rdparty/libxmp" "${CMAKE_BINARY_DIR}/libxmp" EXCLUDE_FROM_ALL)
    target_link_libraries(openomf::xmp INTERFACE xmp_static)
    target_include_directories(openomf::xmp INTERFACE "${CMAKE_SOURCE_DIR}/3rdparty/libxmp/include")
elseif(VCPKG_TOOLCHAIN)
...
if(EXISTS "${CMAKE_SOURCE_DIR}/3rdparty/confuse/confuse.c")
    add_library(vendored_confuse STATIC
        "${CMAKE_SOURCE_DIR}/3rdparty/confuse/confuse.c"
        "${CMAKE_SOURCE_DIR}/3rdparty/confuse/lexer.c"
    )
    target_include_directories(vendored_confuse PUBLIC "${CMAKE_SOURCE_DIR}/3rdparty/confuse")
    target_compile_definitions(vendored_confuse PRIVATE HAVE_CONFIG_H=1)
    target_link_libraries(openomf::confuse INTERFACE vendored_confuse)
    target_include_directories(openomf::confuse INTERFACE "${CMAKE_SOURCE_DIR}/3rdparty/confuse")
elseif(VCPKG_TOOLCHAIN)
...
if(USE_OPUSFILE)
    ...
    if(VCPKG_TOOLCHAIN)
        ...
        if(CMAKE_SYSTEM_NAME STREQUAL "NintendoSwitch")
            target_link_libraries(openomf::opusfile INTERFACE opus ogg)
        endif()
...
if(CMAKE_SYSTEM_NAME STREQUAL "NintendoSwitch")
    add_library(openomf::epoxy INTERFACE IMPORTED)
    target_include_directories(openomf::epoxy INTERFACE "${CMAKE_SOURCE_DIR}/3rdparty/epoxy")
    target_link_libraries(openomf::epoxy INTERFACE glad EGL glapi drm_nouveau nx)
else()
